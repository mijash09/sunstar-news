...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $totalViewsCount = Article::sum('views_count') + 125400;
    ...
    public function store(Request $request)
    {
        ...
        switch ($action) {
            ...
            case 'add-article':
                ...
                if (!empty($rawImageUrl) && !str_starts_with($rawImageUrl, '[object')) {
                    ...
                    foreach ($urlParts as $p) {
                        $p = trim($p);
                        if (!empty($p) && !in_array($p, $imagesList, true)) {
                            $imagesList[] = $p;
                        }
                    }
                }
            ...
            case 'update-article':
                ...
                // Put newImages first so freshly uploaded photos become active covers unless an explicit primary is set
                ...
                if (!empty($files)) {
                    ...
                    foreach ($files as $idx => $file) {
                        if ($file && $file->isValid()) {
                            ...
                            $bTitle = $total > 1 ? ($title . ' (' . ($idx + 1) . ')') : $title;
